  ...
  Future<List<String>> getSuggestions(String word) async {
    ...
    try {
      ...
      if (collegiateResponse.statusCode == 200) {
        ...
        if (collegiateJsonResponse.isNotEmpty &&
            collegiateJsonResponse[0] is String) {
          List<String> suggestions = List<String>.from(collegiateJsonResponse);

          // Find and print the closest suggestion
          String closestSuggestion =
              StringSimilarity.findBestMatch(word, suggestions)
                  .bestMatch
                  .target!;
          print(
              'Closest suggestion for "$word" in collegiate dictionary is "$closestSuggestion".');
        ...
        print(
            'Failed to fetch suggestions for "$word" from collegiate dictionary. Status code: ${collegiateResponse.statusCode}');
      }
    } catch (e) {
      print(
          'Error fetching suggestions for "$word" from collegiate dictionary: $e');
    }
    ...
    try {
      ...
      if (medicalResponse.statusCode == 200) {
        ...
        // If the response is a list of strings, it contains suggestions
        if (medicalJsonResponse.isNotEmpty &&
            medicalJsonResponse[0] is String) {
          ...
          String closestSuggestion =
              StringSimilarity.findBestMatch(word, suggestions)
                  ...
                  .target!;
          print(
              'Closest suggestion for "$word" in medical dictionary is "$closestSuggestion".');
        ...
        print(
            'Failed to fetch suggestions for "$word" from medical dictionary. Status code: ${medicalResponse.statusCode}');
      }
    } catch (e) {
      print(
          'Error fetching suggestions for "$word" from medical dictionary: $e');
    }
